	...
	$( "#login_form" ).dialog({autoOpen: false, modal: true, width:240, draggable: true,
		...
		buttons: {
			"Login": function() {
				...
				bValid = bValid && checkLength( login_form_user, "Por favor revisa tu nombre de usuario", 3, 50 );
				bValid = bValid && checkLength( login_form_password, "Please revisa tu password", 3, 16 );
				if ( bValid ) {
					...
					$.ajax({
						...
						success: function(logged) {
							...
							if(logged==true){
								...
								updateTips("Login exitoso, redireccionandote a tu cuenta",1)
							}
							if(logged==false){
								updateTips( "Por favor, revisa tus datos de acceso",0 );
							}
