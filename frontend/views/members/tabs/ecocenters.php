...
<div id="profilecontent" class="ecocenters clearfix" >
	<h3>My Ecocenters
	<?php if ($myprofile): ?>
		<button class="green addecocenter fright" onclick="location.href='<?php echo _VIEWS_URL_ ?>_registration/ecocenter_reg.php'">Add Ecocenter</button>
	<?php endif ?>
	</h3>
	...
	if($ecocenters){
		...
		<div class="ecocenter">
			<memberavatar class="margin10r"><?php ec_display_profile_thumb($id); ?></memberavatar>
      <div class="fleft">
        <h3><a href="<?php echo _VIEWS_URL_ ?>ecocenters/profile.php?ec_id=<?php echo $id ?>"><?php echo $name ?></a></h3>
        <input type="hidden" name="ec_id" value="<?php echo $id ?>" id="ec_id">
        <input type="hidden" name="role_id" value="<?php echo $roleid ?>" id="role_id">
        <div class="role"><icon/><?php echo $rolename ?></div>
        <div class="address"><icon/><?php echo $address ?></div>
        <div class="description"><icon/><?php echo $description ?></div>
      </div>
		</div>
	<?php } // end foreach
	} // if ecocenters?>
