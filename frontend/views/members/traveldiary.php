<?php
	...
	<div class="wrapper" id="travel">
		<div id="content" class="clearfix">

	<button class="green back iconspan margin10" onclick="location.href='?user_id=<?php echo $userid?>'"><span class="ui-icon ui-icon-arrowthick-1-w"></span>Back to trips</button>
	<?php 
	// Displays only one trip Display This only if the variable _GET['tripid'] is set
      ...
      <div id="skills" class="relative fleft clearfix"><div class="padding10l">
      	<div id="friendsbar">
          <div>
          ...
          <div id="buttons">
            ...
            <?php if (members_is_invited_to_trip($_SESSION['user_id'], $tripid)){
                if (!members_is_confirmed_to_trip($_SESSION['user_id'], $tripid)){?>
                  <button class="green acceptinvite">Join Trip</button>
                  <button class="green leavetrip">Reject</button>
                <?php }else{ ?>
				...
				if($tripstops){
					...
					<div class="tripstop" id="stop_<?php echo $id ?>" geo-lat="<?php echo $latitude ?>" geo-long="<?php echo $longitude; ?>">
						...
						<div class="stopcontent">
							<memberavatar class="tiptip stopauthor" title="<?php echo members_get_info('nombre',$userfrom). " ". members_get_info('apellido',$userfrom);?>"><?php members_display_profile_thumb($userfrom)?></memberavatar>
						...
						<?php $skills = members_get_trip_skills($tripid);
							if($skills){
								foreach ($skills as $key => $value) { 
									...
									$petalclass = $skills[$key]['petalclass'];
									?>            
									<div class="tiptip skill fleft <?php echo $petalclass ?>" title = "<?php echo $petalname ?>"><?php echo $skillname?></div>
						<?php	}// Enf foreach
							} // End if?>
					</div>
					<script>
