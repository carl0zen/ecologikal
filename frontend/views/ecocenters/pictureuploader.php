...
$(function() {
	...
	$("#uploader").pluploadQueue({
		...
		<?php if ($type == 'ec_album'){ ?>
			url: BACKEND_URL + 'ecocenters/pictureuploader.php?type=ec_album&ec_id=<?php echo $ec_id ?>&albumname=<?php echo $albumname?>',
		<?php } ?>
		max_file_size: '10mb',
		chunk_size: '1mb',
		unique_names: true,
		...
		init: {
			...
			<?php if ($type == 'ec_album'){ ?>
				if(up.state == plupload.STOPPED){
					window.location="<?php echo _VIEWS_URL_ ?>ecocenters/picturedetails.php?type=ec_album&ec_id=<?php echo $ec_id ?>&albumname=<?=$albumname;?>";
				}
			<?php }	?>
			<?php if ($type == 'room'){ ?>
				if(up.state == plupload.STOPPED){
