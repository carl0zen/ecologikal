...
1. `/login` → **Demo local** as `guest1` / guest.
2. `/profile` → declare skill *Permacultura*, petal Tierra, level 3.
3. Show flower bars + KINS +3.

### 3. Host + eco-center (2m)

1. Logout → login as `host1` / host.
2. `/admin` → create guest account `guest2` (NC warning OK if compose not up).
...
1. `/profile` — references + verified badges.
2. `/play` — ledger shows skill / reference / volunteer earns.
3. `/cooperate` — vacancy shows completion.
