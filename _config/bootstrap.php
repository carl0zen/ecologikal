...
include_once(_ROOT_PATH_."backend/functions_ecocenter.php");
include_once(_ROOT_PATH_."backend/functions_place.php");
include_once(_ROOT_PATH_."backend/functions_need.php");
include_once(_ROOT_PATH_."backend/functions_trip.php");
...
$url = get_url();
if ($url != 'index.php' && $url != 'login_form.php' && $url != 'login.php' && $url != 'member_reg.php' && $url != 'memberdata.php'){
	if (!is_logged_in()){
		session_start();
		$_SESSION['user_id'];
		...
		ob_start();
		header("location: "._ROOT_URL_."index.php"); // Back to login form
		ob_end_flush();
	}
}
if ($url == 'index.php' && is_logged_in()){
	header("location: "._VIEWS_URL_."members/member_profile.php");
}
